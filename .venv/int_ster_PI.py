...
for i in range(0, N):
    ...
    u.append(min(u_max, max(u_min, u_temp)))  # zamiast kożystać z 2 "if'ów" wykonujemy min max aby wartość
    # przekazana do elementu wykonawczego była realna czyli 0<u<10 tak jak w poleceniu
    q_d = (((u[-1] - u_min) / (u_max - u_min)) * (q_max - q_min) + q_min)  # po obliczeniu wartości sterującej
    Q.append(q_d)
    # wyznaczamy wartość natężenia dopływu
    H.append(min(h_max, max(h_min, t_p * (q_d - B * sqrt(H[-1])) / A + H[-1])))  # symulacja napełniania zbiornika,
